...
// ---------------------------------------------------------------------------
// AncestorBreadcrumb — sits above EntityHeader, shows ancestor › chain
// ---------------------------------------------------------------------------
...
export function AncestorBreadcrumb({ entityId }: AncestorBreadcrumbProps) {
  useEffect(() => { injectCss(); }, []);
  const navigate = useNavigate();
  ...
  const ancestors = (data?.path ?? []).filter((a) => a.id !== entityId);
  if (ancestors.length === 0) return null;

  const showEllipsis = ancestors.length > MAX_ANCESTORS;
  const visible = showEllipsis ? ancestors.slice(-MAX_ANCESTORS) : ancestors;
  const hiddenPath = showEllipsis
    ? ancestors.slice(0, ancestors.length - MAX_ANCESTORS).map((a) => a.label).join(' > ')
    : '';
  ...
  return (
    <div style={{
      ...
      {visible.map((anc, i) => (
        <span key={anc.id} style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: i < visible.length - 1 ? 1 : 0, minWidth: 0 }}>
          <button
            onClick={() => navigate(`/entity/${anc.id}`)}
            style={{
              background: 'none',
              border: 'none',
              padding: '2px 6px',
          ...
          {i < visible.length - 1 && (
            <span style={{ fontSize: '11px', color: 'var(--text-ghost)', flexShrink: 0 }}>›</span>
          )}
        </span>
      ))}
      <span style={{ fontSize: '11px', color: 'var(--text-ghost)', flexShrink: 0 }}>›</span>
...
// ---------------------------------------------------------------------------
// ChildTopicsBranch — sits below EntityHeader, shows children with SVG tree
// ---------------------------------------------------------------------------

interface ChildTopicsBranchProps {
  entityId: string;
}

const STEM_HEIGHT  = 20; // vertical stem from header bottom to branch point
const CHILD_GAP    = 8;  // vertical gap between child pills
const CHILD_HEIGHT = 26;
const BRANCH_X     = 40; // horizontal offset from left edge to branch stem center

export function ChildTopicsBranch({ entityId }: ChildTopicsBranchProps) {
  useEffect(() => { injectCss(); }, []);
  const navigate   = useNavigate();
  const containerRef = useRef<HTMLDivElement>(null);
  const svgRef       = useRef<SVGSVGElement>(null);
  ...
  // Imperatively draw SVG tree lines
  useLayoutEffect(() => {
    const container = containerRef.current;
    const svg       = svgRef.current;
    if (!container || !svg) return;
    ...
    function makePath(d: string, opts?: { dasharray?: string; opacity?: number; width?: number }) {
      const p = document.createElementNS(svgNS, 'path') as SVGPathElement;
      p.setAttribute('d', d);
      p.setAttribute('stroke', 'var(--color-topic)');
      p.setAttribute('stroke-width', String(opts?.width ?? 1.5));
      p.setAttribute('fill', 'none');
      p.setAttribute('stroke-linecap', 'round');
      if (opts?.dasharray) p.setAttribute('stroke-dasharray', opts.dasharray);
      p.setAttribute('opacity', String(opts?.opacity ?? 0.3));
      svg.appendChild(p);
    }

    // Vertical stem from top of container down to branch point
    // Branch point y = STEM_HEIGHT
    makePath(`M ${BRANCH_X},0 L ${BRANCH_X},${STEM_HEIGHT}`, { opacity: 0.4, width: 2 });

    // For each child: draw horizontal line from stem to child left edge, then connect at midY
    for (const el of childEls) {
      const er = el.getBoundingClientRect();
      const childMidY = er.top + er.height / 2 - cr.top;
      const childLeft = er.left - cr.left;

      // Vertical portion of stem to this child's midY
      // Horizontal line from stem x to child left
      const d = `M ${BRANCH_X},${STEM_HEIGHT} L ${BRANCH_X},${childMidY} L ${childLeft},${childMidY}`;
      makePath(d, { opacity: 0.25 });
    }

    // Vertical spine from STEM_HEIGHT to last child's midY
    const lastEl = childEls[childEls.length - 1];
    if (lastEl && childEls.length > 1) {
      const lastEr = lastEl.getBoundingClientRect();
      const lastMidY = lastEr.top + lastEr.height / 2 - cr.top;
      const firstEr = childEls[0]!.getBoundingClientRect();
      const firstMidY = firstEr.top + firstEr.height / 2 - cr.top;
      makePath(`M ${BRANCH_X},${firstMidY} L ${BRANCH_X},${lastMidY}`, { opacity: 0.35, width: 1.5 });
    }
  });
  ...
  useEffect(() => {
    ...
    if (!container || !svg) return;
    const ro = new ResizeObserver(() => {
      // trigger re-layout via forceUpdate pattern — just set svg size
      svg.setAttribute('width', String(container.offsetWidth));
    });
    ro.observe(container);
    return () => ro.disconnect();
  ...
  return (
    <div
      ...
      style={{
        position: 'relative',
        marginTop: '-16px', // overlap EntityHeader marginBottom to close the gap
        marginBottom: '16px',
        paddingTop: `${STEM_HEIGHT}px`,
        paddingLeft: `${BRANCH_X + 16}px`,
      }}
    >
      {/* SVG overlay */}
      <svg
      ...
      {/* Child pills */}
      <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', gap: `${CHILD_GAP}px` }}>
        {visibleChildren.map((child) => (
          <ChildPill
            key={child.id}
            child={child}
